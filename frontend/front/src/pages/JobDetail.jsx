...
//     setIsFetching(true);
//     try {
//     if(!window.confirm('Delete Jop Post?')) {
//       const res = await apiRequest({
//        url: "/job/delete-jop/"+job?._id,
//        token: user?.token,
...
//   return (
    // <div className="container-fluied mx-auto bg-black">
    //   <div className="w-full flex flex-col md:flex-row gap-10 bg-black">
    //     {/* LEFT SIDE */}
    //     {isFetching?(<Loading/>):
...
const JobDetail = () => {
  ...
  return (
    <div class="container-fluid mx-auto bg-black">
    <div class="w-full flex flex-col md:flex-row gap-10 bg-black">
      <div class="w-full h-fit md:w-2/3 2xl:w-2/4 bg-white px-5 py-10 md:px-10 shadow-md">
        <div class="w-full flex items-center justify-between">
          <div class="w-3/4 flex gap-2">
            ...
            <div class="flex flex-col">
              <p class="text-xl font-semibold text-gray-600">{job?.jobTitle}</p>
              <span class="text-base">{job?.location}</span>
              <span class="text-base text-blue-600">{job?.company?.name}</span>
              <span class="text-gray-500 text-sm">{moment(job?.createdAt).fromNow()}</span>
            </div>
          </div>

          <div>
            <AiOutlineSafetyCertificate class="text-3xl text-blue-500" />
          </div>
        </div>

        <div class="w-full flex flex-wrap md:flex-row gap-2 items-center justify-between my-10">
          <div class="bg-[#bdf4c8] w-40 h-16 rounded-lg flex flex-col items-center justify-center">
            <span class="text-sm">Salary</span>
            <p class="text-lg font-semibold text-gray-700">$ {job?.salary}</p>
          </div>

          <div class="bg-[#bae5f4] w-40 h-16 rounded-lg flex flex-col items-center justify-center">
            <span class="text-sm">Job Type</span>
            <p class="text-lg font-semibold text-gray-700">{job?.jobType}</p>
          </div>

          <div class="bg-[#fed0ab] w-40 h-16 rounded-lg flex flex-col items-center justify-center">
            <span class="text-sm">No. of applications</span>
            <p class="text-lg font-semibold text-gray-700">{job?.application?.length}</p>
          </div>

          <div class="bg-[#cecdff] w-40 h-16 rounded-lg flex flex-col items-center justify-center">
            <span class="text-sm">No. of Vacancies</span>
            <p class="text-lg font-semibold text-gray-700">{job?.vacancies}</p>
          </div>

          <div class="bg-[#cecdff] w-40 h-16 rounded-lg flex flex-col items-center justify-center">
            <span class="text-sm">Yr. of Experience</span>
            <p class="text-lg font-semibold text-gray-700">{job?.experience}</p>
          </div>
        </div>
            ...
            <>
              ...
              {job?.detail[0]?.requirements && (
                <>
                  ...
                  <span class="text-base">{job?.detail[0]?.requirements}</span>
                </>
              )}
            </>
          ) : (
